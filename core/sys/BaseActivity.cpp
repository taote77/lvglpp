...
// #include "heygears/utils/logger/Logger.h"
...
{
    // hDebug() << "BaseActivity onCreate,class name:" << typeid(*this).name();
    // 获取当前活动的窗口
...
{
    // hDebug() << "BaseActivity onResume,class name:" << typeid(*this).name();
    if (window_ptr != nullptr) {
...
{
    // hDebug() << "BaseActivity onDestroy,class name:" << typeid(*this).name();
    if (window_ptr != nullptr) {
        delete window_ptr;
        window_ptr = nullptr;
    }
