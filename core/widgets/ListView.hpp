...
#ifndef LVGL_XFJ_DEMO_LISTVIEW_H
#define LVGL_XFJ_DEMO_LISTVIEW_H
...
namespace heygears
{
    namespace widgets
    {
        ...
        public:
            ...
            int getDelegateHeight() const
            {
                return delegateHeight_;
            }

            void setAsyncLoad(bool b)
            {
                async_load_ = b;
            }
        ...
        template<typename T, typename T2>
        ListView<T, T2>::ListView(const BaseModel<T> &model, BaseItem *parentItem):ScrollItem(parentItem),
                                                                                   list_view_model_(model)
        {
            initItem();
            //update();
        ...
        void ListView<T, T2>::update()
        {
            if (delegateHeight_ == -1)
            {
                ...
                if (async_load_ && list_view_model_.count() > 5)
                {
                    ...
                    async_load_timer_ = lv_timer_create([](lv_timer_t *t) -> void
                                                        {
                                                            auto p = (ListView *) t->user_data;
                                                            if (p != nullptr)
                                                            {
                                                                int start_index = p->show_content_vec_.size();
                                                                int end_index = std::min(start_index + 5,
                                                                                         p->list_view_model_.count() -
                                                                                         1);
                                                                if (end_index >= start_index)
                                                                {
                                                                    ...
                                                                    //LV_LOG_USER("async load item from %d to %d",start_index,end_index);
                                                                }
                                                            }
                                                            if (t->repeat_count == 0)
                                                            {
            ...
            else
            {
                ...
                {
                    last_index = std::min((refresh_page_index_ + 2) * page_item_count, list_view_model_.count() - 1);;
                }
                else
                {
                    first_index = std::min((prev_page_index + 2) * page_item_count + 1, list_view_model_.count() - 1);
                    last_index = std::min((refresh_page_index_ + 2) * page_item_count, list_view_model_.count() - 1);
                }
                ...
                //show_content_vec_.clear();
                //LV_LOG_USER("refresh item from %d to %d,prev index:%d,index:%d",first_index,last_index,prev_page_index,refresh_page_index_);
        ...
        void ListView<T, T2>::initItem()
        {
            ...
            lv_obj_add_event_cb(getLvglItem(), [](lv_event_t *e) -> void
            {
...
#endif //LVGL_XFJ_DEMO_LISTVIEW_H
