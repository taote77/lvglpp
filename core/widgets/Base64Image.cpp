...
void Base64Image::updateImage(const std::string &data)
{
    ...
    dsc.data = (const uint8_t *)img_data.data();
    dsc.data_size = img_data.size();
    dsc.header.w = w_;
    dsc.header.h = h_;
    ...
    // if (lv_image_decoder_open(&decoder_dsc, &dsc, LV_COLOR_FORMAT_ARGB8888) != LV_RES_OK) {
    //     std::cerr << "Failed to decode PNG" << std::endl;
    //     return;
    // }

    // // 获取解码后的图像数据
    // img_struct_.header.w = decoder_dsc.decoded.w;
    // img_struct_.header.h = decoder_dsc.decoded.h;
    // img_struct_.header.cf = decoder_dsc.decoded.cf;
    // img_struct_.data = decoder_dsc.decoded.data;
    // img_struct_.data_size = decoder_dsc.decoded.data_size;

    // // 设置图像源
    // lv_image_set_src(getLvglItem(), &img_struct_);
    ...
    // lv_png_decode_rgba32(&output_buf, &decode_w, &decode_h, (unsigned char *)img_data.data(),
    //                      img_data.length());
    ...
    // img_struct_.header.reserved_2 = 0;
