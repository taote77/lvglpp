/**************************************************************************

Copyright:Copyright © 2022 HeyGears. All rights reserved

Author: LnJan

Date:2022-07-12

Class:${CLASS}

Description:

**************************************************************************/

#include <unistd.h>
#include "BaseDialog.h"
#include "GlobalVar.h"
#include "DialogsEventManager.h"

namespace heygears {
namespace widgets {
BaseItem *BaseDialog::getRootItem() const
{
    return background_item_ptr_.get();
}

BaseDialog::BaseDialog(bool is_sys_dialog)
    : background_item_ptr_(new BaseItem(is_sys_dialog ? BaseItem::SysDialog : BaseItem::Dialog)),
      is_sys_dialog_(is_sys_dialog)
{
    background_item_ptr_->setBgColor(CLR_PRIMARY_BLACK);
    background_item_ptr_->setOpacity(0.3);
    background_item_ptr_->setSize(1024, 600);
    background_item_ptr_->setRadius(0);
    background_item_ptr_->setVisible(false);
}

void BaseDialog::open()
{
    if (background_item_ptr_->getVisible())
        return;
    if (!is_sys_dialog_)
        lv_obj_send_event(lv_layer_top(), DialogsEventManager::getInstance()->getOpenEventId(),
                          nullptr);
    background_item_ptr_->setVisible(true);
}

void BaseDialog::close()
{
    if (!background_item_ptr_->getVisible())
        return;
    if (!is_sys_dialog_)
        lv_obj_send_event(lv_layer_top(), DialogsEventManager::getInstance()->getCloseEventId(),
                          this);
    background_item_ptr_->setVisible(false);
    if (auto_del_) {
        delete this;
    }
}

bool BaseDialog::isOpen()
{
    return background_item_ptr_->getVisible();
}

BaseDialog::~BaseDialog()
{
    if (background_item_ptr_->getVisible() && !is_sys_dialog_) {
        lv_obj_send_event(lv_layer_top(), DialogsEventManager::getInstance()->getCloseEventId(),
                          nullptr);
    }
}
} // namespace widgets
} // namespace heygears