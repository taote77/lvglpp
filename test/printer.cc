...
static void menu_bar_create(lv_obj_t *main_cont);
...
void lv_demo_ebike(lv_obj_t *main_cont)
{
    ...
    lv_subject_add_observer_obj(&ebike_subject_language, language_observer_cb, lv_screen_active(),
                                NULL);
    ...
    lv_obj_set_flex_flow(lv_screen_active(),
...
struct EventAdapter
{
    std::function<void(lv_event_t *)> func;

    static void adapter_callback(lv_event_t *e)
    {
        EventAdapter *adapter = static_cast<EventAdapter *>(lv_event_get_user_data(e));
...
struct LanguageEventAdapter
{
    std::function<void(lv_event_t *)> func;

    static void adapter_callback(lv_event_t *e)
    {
        LanguageEventAdapter *adapter =
                static_cast<LanguageEventAdapter *>(lv_event_get_user_data(e));
        if (adapter && adapter->func) {
            adapter->func(e);
        }
    }
...
static void menu_bar_create(lv_obj_t *main_cont)
{
    ...
    lv_obj_set_flex_flow(menu_cont,
    ...
    void menu_item_click_event_cb(lv_event_t * e);

    auto menu_item_click_evt_cb = [&main_cont](lv_event_t *e) -> void {
    ...
    // EventAdapter adapter{ menu_item_click_evt_cb };
...
static void menu_item_click_event_cb(lv_event_t *e)
{
    ...
    switch (idx) {
    case 0:
        lv_demo_ebike_settings_create(ref_main);
        break;
    case 1:
        lv_demo_ebike_stats_create(ref_main);
        break;
    case 2:
        lv_demo_ebike_home_create(ref_main);
        break;
    default:
        break;
    }
