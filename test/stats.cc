...
static int32_t top_speed_values[] = { 46, 28, 42, 39, 41, 25, 49, 37, 35, 40, 33, 40, 31, 27, 45,
                                      38, 41, 40, 27, 25, 30, 45, 31, 43, 41, 34, 47, 32, 30, 33 };
static int32_t avg_speed_values[] = { 21, 24, 27, 29, 23, 28, 28, 22, 29, 28, 24, 26, 24, 30, 25,
                                      25, 20, 28, 24, 27, 25, 27, 20, 29, 25, 24, 23, 26, 27, 27 };
static int32_t distance_values[] = { 87, 63, 29, 84, 27, 84, 33, 76, 77, 49, 46, 29, 67, 21, 87,
                                     75, 40, 19, 12, 67, 66, 11, 59, 33, 51, 75, 44, 61, 53, 63 };
...
void lv_demo_ebike_stats_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_flex_flow(main_cont,
                         LV_DEMO_EBIKE_PORTRAIT ? LV_FLEX_FLOW_COLUMN : LV_FLEX_FLOW_ROW);
...
static void current_week_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    if (week == 0)
        lv_obj_set_style_image_opa(left_arrow, LV_OPA_50, 0);
    else
        lv_obj_set_style_image_opa(left_arrow, LV_OPA_100, 0);

    if (week == 3)
        lv_obj_set_style_image_opa(right_arrow, LV_OPA_50, 0);
    else
        lv_obj_set_style_image_opa(right_arrow, LV_OPA_100, 0);
}
...
static void current_data_objserver_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    LV_UNUSED(subject);
    lv_obj_t *label = lv_observer_get_target_obj(observer);
    stat_mode_t mode = static_cast<stat_mode_t>(lv_subject_get_int(&subject_mode));
...
{
    lv_obj_t *chart = static_cast<lv_obj_t *>(lv_event_get_target(e));
...
static void chart_draw_task_event_cb(lv_event_t *e)
{
    ...
    lv_draw_dsc_base_t *base_dsc = static_cast<lv_draw_dsc_base_t *>(draw_task->draw_dsc);
    ...
    lv_draw_line_dsc_t *draw_line_dsc = static_cast<lv_draw_line_dsc_t *>(draw_task->draw_dsc);
    ...
    tri_dsc.p[2].x =
            draw_line_dsc->p1.y < draw_line_dsc->p2.y ? draw_line_dsc->p1.x : draw_line_dsc->p2.x;
    tri_dsc.p[2].y = LV_MAX(draw_line_dsc->p1.y, draw_line_dsc->p2.y);
    tri_dsc.bg_grad.dir = LV_GRAD_DIR_VER;

    int32_t full_h = lv_obj_get_height(obj);
    int32_t fract_upper =
            (int32_t)(LV_MIN(draw_line_dsc->p1.y, draw_line_dsc->p2.y) - obj->coords.y1) * 255
            / full_h;
    int32_t fract_lower =
            (int32_t)(LV_MAX(draw_line_dsc->p1.y, draw_line_dsc->p2.y) - obj->coords.y1) * 255
            / full_h;
...
static lv_obj_t *chart_create(lv_obj_t *parent)
{
    ...
    lv_chart_series_t *ser = lv_chart_add_series(chart, lv_color_white(), lv_chart_axis_t(0));
...
static lv_obj_t *stat_card_create(lv_obj_t *parent, const char *name, lv_subject_t *subject,
                                  const char *fmt)
{
    lv_obj_t *cont = lv_obj_create(parent);
    lv_obj_set_flex_flow(cont, LV_FLEX_FLOW_COLUMN);
