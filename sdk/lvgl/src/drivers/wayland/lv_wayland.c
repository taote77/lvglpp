                          ...
                          uint32_t name, const char * interface, uint32_t version)
{
    ...
    // log interface name
    LV_LOG_TRACE("###########interface: %s", interface);
    LV_LOG_TRACE("############## wl_compositor_interface: %s", wl_registry_interface.name);
    LV_LOG_TRACE("############## wl_subcompositor_interface: %s", wl_subcompositor_interface.name);
    LV_LOG_TRACE("############## wl_shm_interface: %s", wl_shm_interface.name);
    LV_LOG_TRACE("############## wl_seat_interface: %s", wl_seat_interface.name);
    LV_LOG_TRACE("############## wl_seat_interface: %s", wl_shell_interface.name);
...
static void wayland_init(void)
{
    ...
    if(application.compositor == NULL) {
        LV_LOG_ERROR("Wayland compositor not available");
        return;
    }
    ...
    if(application.shm == NULL) {
        LV_LOG_ERROR("Wayland SHM not available");
        return;
    }
    ...
    if(application.shm_format == SHM_FORMAT_UNKNOWN) {
        ...
        LV_LOG_ERROR("Unable to match a suitable SHM format for selected LVGL color depth");
        return;
    }
