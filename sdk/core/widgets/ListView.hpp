...
template <typename T, typename T2>
ListView<T, T2>::ListView(const BaseModel<T> &model, BaseItem *parentItem) : ScrollItem(parentItem), list_view_model_(model)
{
    initItem();
    // update();
            ...
            async_load_timer_ = lv_timer_create(
                [](lv_timer_t *t) -> void {
                    ...
                    if (p != nullptr)
                    {
                        ...
                        int end_index   = std::min(start_index + 5, p->list_view_model_.count() - 1);
