...
static const lv_font_t *getFontByStyle(ConsumeText::TextStyle style, ConsumeText::FontType type)
{
    ...
    case ConsumeText::TextStyle::FontSize22:
    ...
    case ConsumeText::TextStyle::FontSize22Bold:
    ...
    case ConsumeText::TextStyle::FontSize48Bold:
...
ConsumeText::ConsumeText(const std::string &strMsg, uint32_t clr, BaseItem *parent, ConsumeText::TextStyle style, FontType type) :
...
//        {
//            if (sys::Application::getLanguageType() != sys::Application::Chinese2English ||
...
void ConsumeText::setTextOpa(lv_opa_t opa)
{
...
int ConsumeText::getDisplayTextWidth() const
{
    ...
    if (!text_.empty())
    {
        ...
        for (int i = 0; i < wid_string.length(); i++)
        {
            ...
            {
                res += lv_font_get_glyph_width(lv_obj_get_style_text_font(getLvglItem(), LV_STATE_DEFAULT), wid_string[i], wid_string[i + 1]);
            } else
            {
                res += lv_font_get_glyph_width(lv_obj_get_style_text_font(getLvglItem(), LV_STATE_DEFAULT), wid_string[i], 0);
            }
        }
    }
...
void ConsumeText::setLongMode(int mode) // 1-LV_LABEL_LONG_BREAK, 2-LV_LABEL_LONG_SCROLL
{
    if (mode == 1)
    {
...
std::string ConsumeText::getLongModeStr(std::string src)
{
    ...
    if (long_mode_.mode == LongMode::AutoEllipsis)
    {
        ...
        {
            src_text_width += lv_font_get_glyph_width(lv_obj_get_style_text_font(getLvglItem(), LV_STATE_DEFAULT), src_str[i], i < src_str.length() - 1 ? src_str[i + 1] : 0);
            if (src_text_width > real_txt_width && i > 0)
            {
                last_char_index = i;
                out_str         = lvglpp::tools::Utils::ws2s(src_str.substr(0, last_char_index) + lvglpp::tools::Utils::s2ws("..."));
                break;
            }
        }
    } else if (long_mode_.mode == LongMode::MiddleEllipsis)
    {
        ...
        for (int i = 0; i < src_str.length(); i++)
        {
            src_text_width_font += lv_font_get_glyph_width(lv_obj_get_style_text_font(getLvglItem(), LV_STATE_DEFAULT), src_str[i], i < src_str.length() - 1 ? src_str[i + 1] : 0);
            if (src_text_width_font >= real_txt_width / 2 && i > 0)
            {
                last_char_index = i - 1;
                out_str         = lvglpp::tools::Utils::ws2s(src_str.substr(0, last_char_index + 1) + lvglpp::tools::Utils::s2ws("..."));
                break;
            }
        }
        ...
        {
            src_text_width_end += lv_font_get_glyph_width(lv_obj_get_style_text_font(getLvglItem(), LV_STATE_DEFAULT), src_str[i], i < src_str.length() - 1 ? src_str[i + 1] : 0);
        ...
        out_str += lvglpp::tools::Utils::ws2s(src_str.substr(last_char_index, src_str.length() - last_char_index));
    }
    return out_str;
}
} // namespace lvglpp::widgets
